...
{
    uint3 id;
    _GET_GL_GLOBAL_INVOCATION_ID(uint3,id);

    uint width,height;
    GetRWTex2DDimensions_2( Destination,width, height);
    ...
    imageStore( Destination, _ToIvec(id.xy), _ExpandVector( lerp(result, 1, blackAndWhite) * rim));
}